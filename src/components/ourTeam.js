...
class OurTeam extends React.Component {
  ...
  isLogoAlreadyShown(src) {
    return this.state.displayHeadshots.find(
      logo => logo.node.childImageSharp.fluid.src === src
    )
  }
      ...
      <div {...container}>
        ...
        <div {...headshotStyles}>
          {this.state.displayHeadshots.map((file, i) => {
            const element = (
              <TitledHeadshot
                key={file.node.childImageSharp.fluid.src}
                fluid={file.node.childImageSharp.fluid}
                name={getTeamMemberName(file.node.name)}
                title={getTeamMemberTitle(file.node.name)}
              />
            )
            return (
              <BlurOnEnterAndExit
